...
pub fn read_movies(
    ...
    let statement = db.prepare("select * from movies")?;

    let movies: Vec<Movie> = db
        .query(&statement, &[])?
        .iter()
        .map(|row| {
...
pub fn read_movie(
    ...
    let statement = db.prepare("select * from movies where title = $1 ")?;

    let movie: Option<Movie> = db
        .query(&statement, &[&title])?
        .iter()
        // we use fold here instead of map because by using fold we're able to tranform many rows
        // into a single row
        .fold(None, |_acc, row| {
            let title: String = row.get("title");
            let genre: String = row.get("genre");
            Some(Movie { title, genre })
        });
    Ok(movie)
}
...
pub fn insert_movie(
    ...
    let statement = db.prepare("insert into movies (title, genre) values ($1, $2)")?;
...
pub fn delete_movie(
    ...
    let statement = db.prepare("delete from movies where title = $1")?;
    // here title will replace $1 above
